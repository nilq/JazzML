...
macro_rules! token {
  ...
  ($tokenizer:expr, $token_type:expr, $accum:expr) => {{
    let tokenizer  = $tokenizer  as &$crate::jazzml::lexer::tokenizer::Tokenizer<'t>;
    let token_type = $token_type as $crate::jazzml::lexer::token::TokenType;
...
impl<'t> Matcher<'t> for IdentifierMatcher {
  fn try_match(&self, tokenizer: &mut Tokenizer<'t>) -> Result<Option<Token>, ()> {
    let first = tokenizer.peek().unwrap();
