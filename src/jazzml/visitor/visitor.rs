...
impl<'v> Visitor<'v> {
  ...
  fn visit_statement(&mut self, statement: &Statement) -> Result<(), ()> {
    ...
    match statement.node {
      ...
      Continue => if self.inside.contains(&Inside::Loop) {
        Ok(())
      } else {
        return Err(
          response!(
  ...
  fn visit_variable(&mut self, variable: &StatementNode, pos: &Pos) -> Result<(), ()> {
    use self::ExpressionNode::*;
     ...
     if let &StatementNode::Variable(ref is_mutable, ref var_type, ref name, ref right) = variable {
  ...
  fn type_expression(&mut self, expression: &Expression) -> Result<Type, ()> {
    ...
    let t = match expression.node {
      ...
      Struct(ref params, ref id) => {
        ...
        Type::new(TypeNode::Struct(param_hash, id.to_string()), TypeMode::Undeclared)
      },
