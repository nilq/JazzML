#ifndef __JAZZ_H__
#define __JAZZ_H__

#ifndef true
    #define true 1
    #define false 0
    typedef int bool;
#endif

#if defined(_WIN32)
    #define WINDOWS
#endif

#if defined(__APPLE__)
    #define MAC
#endif



#if defined(linux) || defined(__linux__)
    #define LINUX
#endif

#if defined(__GNUC__)
    #define GCC
#endif

#if defined(_MSC_VER)
    #define VCC
#endif

#if defined(__i386__) || defined(_WIN32)
    #define X86
#endif

#if defined(_64BITS) || defined(__x86_64__)
    #define J64BITS
#endif


#if defined(LINUX) || defined(MAC)
    #define POSIX
#endif

#if defined(GCC)
    #define THREADED
    #define DIRECT_THREADED
#endif

#include <stddef.h>
#ifndef VCC
    #include <stdint.h>
#endif


bool IS_LITTLEENDIAN() {
    short int number = 0x1;
    char *numPtr = (char*)&number;
    return (numPtr[0] == 1);
}

#if !defined(MAC) && !defined(_WIN64)
    #define JIT_ENABLE
#endif

typedef intptr_t int_val;

typedef intptr_t int_val;

typedef enum {
	VAL_INT			= 0xFF,
	VAL_NULL		= 0,
	VAL_FLOAT		= 1,
	VAL_BOOL		= 2,
	VAL_STRING		= 3,
	VAL_OBJECT		= 4,
	VAL_ARRAY		= 5,
	VAL_FUNCTION	= 6,
	VAL_ABSTRACT	= 7,
	VAL_INT32		= 8,
    VAL_INT64       = 9,
	VAL_PRIMITIVE	= 6 | 16,
	VAL_JITFUN		= 6 | 32,
	VAL_32_BITS		= 0xFFFFFFFF
} val_type;

typedef struct {
    int __zero;
} *value_kind;

typedef struct {
    val_type t;
} *value;

typedef struct {
    int id;
    value v;
} Cell;

typedef struct {
    int count;
    Cell* cells;
} ObjectTable;

struct _Buff;

typedef struct _Buff *Buffer;

typedef void (*finalizer)(value v);

#pragma pack(4)
/// Floating point
typedef struct {
    val_type t;
    double f;
} Float;
/// Signed integer with 32 bits length
typedef struct {
    val_type t;
    int i;
} Int32;

/// Signed integer with 64 bits length
typedef struct {
    val_type t;
    long i;
} Int64;

#pragma pack()
typedef struct Object {
    val_type t;
    ObjectTable table;
    struct Object *proto;
} Object;


typedef struct  {
    val_type t;
    int nargs;
    void *address;
    value env;
    void *module;
} Function;

typedef struct {
    val_type t;
    char c;
} String;

typedef struct {
    val_type t;
    value ptr;
} Array;

typedef struct {
    val_type t;
    value_kind kind;
    void *data;
} Abstract;

typedef struct HashCell {
    int hkey;
    value key;
    value val;
    struct HashCell *next;
} HashCell;    

typedef struct {
    HashCell **cells;
    int ncells;
    int nitems;
} Hash;

struct _mt_local;
struct _mt_lock;
typedef struct _mt_local mt_local;
typedef struct _mt_lock mt_lock;

#define TAG_BITS 4

#define val_tag(v) (*(val_type)(v))


#define val_short_tag(v)	(val_tag(v)&((1<<TAG_BITS) - 1))
#define val_is_null(v)		((v) == val_null)
#define val_is_int(v)		((((int)(int_val)(v)) & 1) != 0)
#define val_is_any_int(v)	(val_is_int(v) || val_tag(v) == VAL_INT32)
#define val_is_bool(v)		((v) == val_true || (v) == val_false)
#define val_is_number(v)	(val_is_int(v) || val_tag(v) == VAL_FLOAT || val_tag(v) == VAL_INT32)
#define val_is_float(v)		(!val_is_int(v) && val_tag(v) == VAL_FLOAT)
#define val_is_int32(v)		(!val_is_int(v) && val_tag(v) == VAL_INT32)
#define val_is_string(v)	(!val_is_int(v) && val_short_tag(v) == VAL_STRING)
#define val_is_function(v)	(!val_is_int(v) && val_short_tag(v) == VAL_FUNCTION)
#define val_is_object(v)	(!val_is_int(v) && val_tag(v) == VAL_OBJECT)
#define val_is_array(v)		(!val_is_int(v) && val_short_tag(v) == VAL_ARRAY)
#define val_is_abstract(v)  (!val_is_int(v) && val_tag(v) == VAL_ABSTRACT)
#define val_is_kind(v,t)	(val_is_abstract(v) && val_kind(v) == (t))
#define val_check_kind(v,t)	if( !val_is_kind(v,t) ) error();
#define val_check_function(f,n) if( !val_is_function(f) || (val_fun_nargs(f) != (n) && val_fun_nargs(f) != VAR_ARGS) ) error();
#define val_check(v,t)		if( !val_is_##t(v) ) error();
#define val_data(v)			((vabstract*)(v))->data
#define val_kind(v)			((vabstract*)(v))->kind

#define val_type(v)			(val_is_int(v) ? VAL_INT : val_short_tag(v))
#define val_int(v)			(((int)(int_val)(v)) >> 1)
#define val_float(v)		(CONV_FLOAT ((Float*)(v))->f)
#define val_int32(v)		(((Int32*)(v))->i)
#define val_any_int(v)		(val_is_int(v)?val_int(v):val_int32(v))
#define val_bool(v)			((v) == val_true)
#define val_number(v)		(val_is_int(v)?val_int(v):((val_tag(v)==VAL_FLOAT)?val_float(v):val_int32(v)))
#define val_hdata(v)		((Hash*)val_data(v))
#define val_string(v)		(&((String*)(v))->c)
#define val_strlen(v)		((signed)(((unsigned)val_tag(v)) >> TAG_BITS))
#define val_set_length(v,l) val_tag(v) = val_short_tag(v) | ((l) << TAG_BITS)
#define val_set_size		val_set_length

#define val_array_size(v)	((signed)(((unsigned)val_tag(v)) >> TAG_BITS))
#define val_array_ptr(v)	(&((Array*)(v))->ptr)
#define val_fun_nargs(v)	((Function*)(v))->nargs
#define alloc_int(v)        ((value)(int_val)((((int)(v)) << 1) | 1))
#define alloc_bool(b)		((b)?val_true:val_false)

#define max_array_size		((1 << (32 - TAG_BITS)) - 1)
#define max_string_size		((1 << (32 - TAG_BITS)) - 1)
#define invalid_comparison	0xFE


#undef EXTERN
#undef EXPORT
#undef IMPORT
#if defined(VCC)
#	define INLINE __inline
#	define EXPORT __declspec( dllexport )
#	define IMPORT __declspec( dllimport )
#else
#	define INLINE inline
#	define EXPORT
#	define IMPORT
#endif

#if defined(SOURCES) || defined(STANDALONE)
#	define EXTERN EXPORT
#else
#	define EXTERN IMPORT
#endif

#define VEXTERN extern EXTERN

#ifdef __cplusplus
#	define C_FUNCTION_BEGIN extern "C" {
#	define C_FUNCTION_END	};
#else
#	define C_FUNCTION_BEGIN
#	define C_FUNCTION_END
#endif

#define need_32_bits(i) ( (((unsigned int)(i)) + 0x40000000) & 0x80000000 )
#define alloc_best_int(i) (need_32_bits(i) ? alloc_int32(i) : alloc_int(i))

#define error()     return NULL;
#define fail(message) _fail(alloc_string(msg),__FILE__,__LINE__)
#define bfail(buf)    _fail(buffer_to_string(buf),__FILE__,__LINE__)

#ifndef CONV_FLOAT
#	define CONV_FLOAT
#endif

#ifdef POSIX
#	include <errno.h>
#	define POSIX_LABEL(name)	name:
#	define HANDLE_EINTR(label)	if( errno == EINTR ) goto label
#	define HANDLE_FINTR(f,label) if( ferror(f) && errno == EINTR ) goto label
#else
#	define POSIX_LABEL(name)
#	define HANDLE_EINTR(label)
#	define HANDLE_FINTR(f,label)
#endif

#define VAR_ARGS (-1)
#define DEFINE_PRIM_MULT(func) C_FUNCTION_BEGIN EXPORT void *func##__MULT() { return (void*)(&func); } C_FUNCTION_END
#define DEFINE_PRIM(func,nargs) C_FUNCTION_BEGIN EXPORT void *func##__##nargs() { return (void*)(&func); } C_FUNCTION_END
#define DEFINE_PRIM_WITH_NAME(func,name,nargs) C_FUNCTION_BEGIN EXPORT void *name##__##nargs() { return (void*)(&func); } C_FUNCTION_END
#define DEFINE_KIND(name) int_val __kind_##name = 0; vkind name = (vkind)&__kind_##name;

#ifdef STANDALONE  
    #define DEFINE_ENTRY_POINT(name)
#else
    #define DEFINE_ENTRY_POINT(name) C_FUNCTION_BEGIN void name(); EXPORT void *_entry_point() { return &name; } C_FUNCTION_END
#endif

#ifdef HEADER_IMPORTS
#	define H_EXTERN IMPORT
#else
#	define H_EXTERN EXPORT
#endif

#define DECLARE_PRIM(func,nargs) C_FUNCTION_BEGIN H_EXTERN void *func##__##nargs(); C_FUNCTION_END
#define DECLARE_KIND(name) C_FUNCTION_BEGIN H_EXTERN extern vkind name; C_FUNCTION_END

#define alloc_int32			alloc_int32
#define alloc_float			alloc_float
#define alloc_string		alloc_string
#define alloc_empty_string	alloc_empty_string
#define copy_string			copy_string
#define val_this			val_this
#define val_id				val_id
#define val_field			val_field
#define alloc_object		alloc_object
#define alloc_field			alloc_field
#define alloc_array			alloc_array
#define val_call0			val_call0
#define val_call1			val_call1
#define val_call2			val_call2
#define val_call3			val_call3
#define val_callN			val_callN
#define val_ocall0			val_ocall0
#define val_ocall1			val_ocall1
#define val_ocall2			val_ocall2
#define val_ocallN			val_ocallN
#define val_callEx			val_callEx
#define	alloc_root			alloc_root
#define free_root			free_root
#define alloc				alloc
#define alloc_private		alloc_private
#define alloc_abstract		alloc_abstract
#define alloc_function		alloc_function
#define alloc_buffer		alloc_buffer
#define buffer_append		buffer_append
#define buffer_append_sub	buffer_append_sub
#define buffer_append_char	buffer_append_char
#define buffer_length		buffer_length
#define buffer_to_string	buffer_to_string
#define val_buffer			val_buffer
#define val_compare			val_compare
#define val_print			val_print
#define val_gc				val_gc
#define val_throw			val_throw
#define val_rethrow			val_rethrow
#define val_iter_fields		val_iter_fields
#define val_field_name		val_field_name
#define val_hash			val_hash
#define k_hash				k_hash
#define kind_share			kind_share
#define kind_lookup			kind_lookup

#define alloc_local			alloc_local
#define local_get			local_get
#define local_set			local_set
#define free_local			free_local
#define alloc_lock			alloc_lock
#define lock_acquire		lock_acquire
#define lock_try			lock_try
#define lock_release		lock_release
#define free_lock			free_lock

C_FUNCTION_BEGIN

	VEXTERN value_kind k_hash;

	VEXTERN value val_null;
	VEXTERN value val_true;
	VEXTERN value val_false;

	EXTERN value alloc_float( double t );
	EXTERN value alloc_int32( int i );

	EXTERN value alloc_string( const char *str );
	EXTERN value alloc_empty_string( unsigned int size );
	EXTERN value copy_string( const char *str, int_val size );

	EXTERN value val_this();
	EXTERN int val_id( const char *str );
	EXTERN value val_field( value o, int f );
	EXTERN value alloc_object( value o );
	EXTERN void alloc_field( value obj, int f, value v );
	EXTERN void val_iter_fields( value obj, void f( value v, int f, void * ), void *p );
	EXTERN value val_field_name( int f );

	EXTERN value alloc_array( unsigned int n );
	EXTERN value alloc_abstract( value_kind k, void *data );

	EXTERN value val_call0( value f );
	EXTERN value val_call1( value f, value arg );
	EXTERN value val_call2( value f, value arg1, value arg2 );
	EXTERN value val_call3( value f, value arg1, value arg2, value arg3 );
	EXTERN value val_callN( value f, value *args, int nargs );
	EXTERN value val_ocall0( value o, int f );
	EXTERN value val_ocall1( value o, int f, value arg );
	EXTERN value val_ocall2( value o, int f, value arg1, value arg2 );
	EXTERN value val_ocallN( value o, int f, value *args, int nargs );
	EXTERN value val_callEx( value vthis, value f, value *args, int nargs, value *exc );

	EXTERN value *alloc_root( unsigned int nvals );
	EXTERN void free_root( value *r );
	EXTERN char *alloc( unsigned int nbytes );
	EXTERN char *alloc_private( unsigned int nbytes );
	EXTERN value alloc_function( void *c_prim, unsigned int nargs, const char *name );

	EXTERN Buffer alloc_buffer( const char *init );
	EXTERN void buffer_append( Buffer b, const char *s );
	EXTERN void buffer_append_sub( Buffer b, const char *s, int_val len );
	EXTERN void buffer_append_char( Buffer b, char c );
	EXTERN value buffer_to_string( Buffer b );
	EXTERN int buffer_length( Buffer b );
	EXTERN void val_buffer( Buffer b, value v );

	EXTERN int val_compare( value a, value b );
	EXTERN void val_print( value s );
	EXTERN void val_gc( value v, finalizer f );
	EXTERN void val_throw( value v );
	EXTERN void val_rethrow( value v );
	EXTERN int val_hash( value v );

	EXTERN void kind_share( value_kind *k, const char *name );
	EXTERN value_kind kind_lookup( const char *name );
	EXTERN void __failure( value msg, const char *file, int line );

	// MULTITHREADING API
	EXTERN mt_local *alloc_local();
	EXTERN void *local_get( mt_local *l );
	EXTERN void local_set( mt_local *l, void *v );
	EXTERN void free_local( mt_local *l );

	EXTERN mt_lock *alloc_lock();
	EXTERN void lock_acquire( mt_lock *l );
	EXTERN int lock_try( mt_lock *l );
	EXTERN void lock_release( mt_lock *l );
	EXTERN void free_lock( mt_lock *l );

C_FUNCTION_END


#endif // __JAZZ_H__